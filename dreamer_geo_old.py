...
def run_one_seed(args, cfg: VariantCfg, seed: int) -> Dict[str, float]:
    ...
    if getattr(args, "checkpoint_dir", None) and args.checkpoint_dir:
        from maze_dreamer_visuals import save_maze_dreamer_checkpoint

        ck_dir = os.path.join(args.checkpoint_dir, run_name)
        os.makedirs(ck_dir, exist_ok=True)
        ck_path = os.path.join(ck_dir, "model_final.pt")
        meta = {
            "env_id": args.env_id,
            "img_size": args.img_size,
            "bit_depth": args.bit_depth,
            ...
            "geo_dim": args.geo_dim,
            "geo_variant": cfg.geo_variant,
            "geo_plan_weight": cfg.geo_plan_weight,
            "geo_shaping_alpha": cfg.geo_shaping_alpha,
        ...
        print(f"    Saved checkpoint: {ck_path}", flush=True)
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="If set, save model_final.pt + meta JSON per variant/seed under this directory",
    )
