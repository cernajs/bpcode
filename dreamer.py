...
def compute_pullback_curvature_loss(
    decoder, h, s, num_projections=4, detach_features=True
):
    """
    ...
    for _ in range(num_projections):
        ...
        Gu_h = torch.autograd.grad(
            outputs=recon,
            inputs=h,
            grad_outputs=Ju_h if isinstance(Ju_h, torch.Tensor) else recon,
            create_graph=True,
            retain_graph=True,
        )[0]
        Gu_s = torch.autograd.grad(
            outputs=recon,
            inputs=s,
            grad_outputs=Ju_s if isinstance(Ju_s, torch.Tensor) else recon,
            create_graph=True,
        )[0]
...
def evaluate_policy(
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            obs, total_reward, term, trunc, _ = env.step(
                action_np, repeat=action_repeat
            )
            done = bool(term or trunc)
...
def build_parser():
    ...
    p.add_argument(
        ...
        type=int,
        default=8,
        help="How many posterior time indices per sequence to start imagination from. Set 0 to use all.",
    )
    p.add_argument(
        "--cont_weight",
        type=float,
        default=1.0,
        help="Weight of continuation (discount) prediction loss in world model.",
    )

    # Exploration
    p.add_argument(
        "--expl_amount", type=float, default=0.3, help="Action noise during collection"
    )
    p.add_argument(
        "--expl_decay", type=float, default=0.0, help="Exploration decay rate"
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Steps before actor training starts",
    )
    p.add_argument(
        "--advantage_clip",
        type=float,
        default=10.0,
        help="Clip advantages before normalization",
    )
...
def main(args):
    ...
    print(
        f"Environment: {args.env_id}, Action repeat: {action_repeat}, effective_gamma: {effective_gamma:.6f}"
    )
    ...
    model_optim = torch.optim.Adam(
        world_model_params, lr=args.model_lr, eps=args.adam_eps
    )
    ...
    replay = ReplayBuffer(
        args.replay_buff_capacity, obs_shape=(H, W, C), act_dim=act_dim
    )
    ...
    for seed_ep in range(args.seed_episodes):
        ...
        print(
            f"  Seed episode {seed_ep + 1}/{args.seed_episodes}: return = {ep_return:.2f}"
        )
    ...
    for episode in range(args.max_episodes):
            ...
            if total_steps % args.collect_interval == 0 and replay.size > (
                ...
                for upd in range(args.train_steps):
                    ...
                    obs_seq = torch.tensor(
                        batch.obs, dtype=torch.float32, device=device
                    )
                    act_seq = torch.tensor(
                        batch.actions, dtype=torch.float32, device=device
                    )
                    rew_seq = torch.tensor(
                        batch.rews, dtype=torch.float32, device=device
                    )
                    ...
                    with autocast(device_type="cuda", enabled=use_amp):
                        ...
                        for t in range(T):
                            h_t_train = rssm.deterministic_state_fwd(
                                h_t_train, s_t_train, act_seq[:, t]
                            )
                            states.append(h_t_train)
                            priors.append(rssm.state_prior(h_t_train))
                            posteriors.append(
                                rssm.state_posterior(h_t_train, e_t[:, t + 1])
                            )
                            post_mean, post_std = posteriors[-1]
                            s_t_train = (
                                post_mean + torch.randn_like(post_std) * post_std
                            )
                        ...
                        if args.bisimulation_weight > 0.0 and T > 2:
                            ...
                            if args.pullback_bisim:
                                ...
                                dz_masked = dz[mask]
                                scale = (
                                    target_masked.detach().abs().mean().clamp_min(1e-6)
                                )
                                bisim_loss_val = F.mse_loss(
                                    dz_masked / scale, target_masked / scale
                                )
                            else:
                                scale = (
                                    target_bisim.detach().abs().mean().clamp_min(1e-6)
                                )
                                bisim_loss_val = F.mse_loss(
                                    dz / scale, target_bisim / scale
                                )
                    ...
                    if use_amp:
                        ...
                        torch.nn.utils.clip_grad_norm_(
                            world_model_params, args.grad_clip_norm
                        )
                    ...
                    with autocast(device_type="cuda", enabled=use_amp):
                        with torch.no_grad():
                            ...
                            if (
                                ...
                                t_idx = torch.randint(
                                    0, T_seq, (B_seq, K), device=device
                                )
                                h_start = (
                                    h_seq.gather(
                                        1, t_idx.unsqueeze(-1).expand(-1, -1, Dh)
                                    )
                                    .reshape(-1, Dh)
                                    .detach()
                                )
                                s_start = (
                                    s_seq.gather(
                                        1, t_idx.unsqueeze(-1).expand(-1, -1, Ds)
                                    )
                                    .reshape(-1, Ds)
                                    .detach()
                                )
                        ...
                        for _ in range(args.imagination_horizon):
                            ...
                            action_im, log_prob = actor.get_action(
                        ...
                        cont_logits_imag = bottle(
                            continue_model, h_imag[:, 1:], s_imag[:, 1:]
                        )  # [B_imag, H]
                        ...
                        with torch.no_grad():
                            ...
                            values_for_targets = bottle(
                                value_model, h_imag, s_imag
                            )  # [B, H+1]
                            lambda_returns_tgt = compute_lambda_returns(
                                rewards_imag.detach(),
                                values_for_targets,
                                discounts_imag.detach(),
                                lambda_=args.lambda_,
                            )
                            weights_for_value = compute_discount_weights(
                                discounts_imag.detach()
                            )
                    ...
                    values_for_value_loss = bottle(
                        value_model, h_imag.detach().float(), s_imag.detach().float()
                    )
                    value_loss = (
                        (values_for_value_loss[:, :-1] - lambda_returns_tgt.float())
                        ** 2
                    ...
                    if total_steps >= args.actor_warmup_steps:
                        with autocast(device_type="cuda", enabled=use_amp):
                            ...
                            actor_loss = -(
                                weights_actor * lambda_returns_actor
                            ).mean() - (actor_entropy_scale * entropy)
                ...
                writer.add_scalar(
                    "loss/model_total",
                    loss_accum["model_total"] / n_updates,
                    total_steps,
                )
                writer.add_scalar(
                    "loss/reconstruction", loss_accum["rec"] / n_updates, total_steps
                )
                writer.add_scalar(
                    "loss/kl_divergence", loss_accum["kl"] / n_updates, total_steps
                )
                writer.add_scalar(
                    "loss/reward", loss_accum["rew"] / n_updates, total_steps
                )
                writer.add_scalar(
                    "loss/continue", loss_accum["cont"] / n_updates, total_steps
                )
                writer.add_scalar(
                    "loss/pullback_curvature", loss_accum["pb"] / n_updates, total_steps
                )
                writer.add_scalar(
                    "loss/bisimulation", loss_accum["bisim"] / n_updates, total_steps
                )
                writer.add_scalar(
                    "loss/actor", loss_accum["actor"] / n_updates, total_steps
                )
                writer.add_scalar(
                    "loss/value", loss_accum["value"] / n_updates, total_steps
                )
                ...
                writer.add_scalar(
                    ...
                    loss_accum["adv_mean"] / n_updates,
                    total_steps,
                )
                writer.add_scalar(
                    "actor/advantage_std",
                    loss_accum["adv_std"] / n_updates,
                    total_steps,
                )
                writer.add_scalar(
                    "actor/advantage_min",
                    loss_accum["adv_min"] / n_updates,
                    total_steps,
                )
                writer.add_scalar(
                    "actor/advantage_max",
                    loss_accum["adv_max"] / n_updates,
                    total_steps,
                )
        ...
        print(
            f"Episode {episode + 1}/{args.max_episodes}: return = {ep_return:.2f}, steps = {ep_steps}, total = {total_steps}"
        )
