...
def log_visualizations(
    ...
    with torch.no_grad():
        ...
        if z_flat.shape[0] > 2:
            ...
            z_2d = z_flat[:, :2] if z_flat.shape[1] >= 2 else np.zeros((z_flat.shape[0], 2))
        ...
        for i in range(n_points):
            for j in knn_indices[i]:
                ...
                if same_batch:
                    ax.plot([z_2d[i, 0], z_2d[j, 0]], [z_2d[i, 1], z_2d[j, 1]], 
        ...
        scatter = ax.scatter(z_2d[:, 0], z_2d[:, 1], c=time_labels, cmap='coolwarm', 
...
"""
        ...
        writer.add_scalar('viz_stats/cross_batch_edge_ratio', cross_batch_edges / total_edges, step)
        writer.add_scalar('viz_stats/same_time_edge_ratio', same_time_edges / total_edges, step)
        writer.add_scalar('viz_stats/pca_variance_explained', sum(pca.explained_variance_ratio_), step)
...
def log_imagination_rollout(
    ...
    with torch.no_grad():
        ...
        mse_per_step = ((imag_recon[0] - obs_batch[0, 1:horizon+1]) ** 2).mean(dim=(1, 2, 3))
        
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.plot(range(1, horizon+1), mse_per_step.cpu().numpy(), 'b-o', label='Imagination MSE')
        ...
        writer.add_scalar('viz_stats/imagination_mse_mean', mse_per_step.mean().item(), step)
        writer.add_scalar('viz_stats/imagination_mse_final', mse_per_step[-1].item(), step)
...
def log_reward_prediction(
    ...
    with torch.no_grad():
        ...
        ax.plot(range(T), pred_rew[0].cpu().numpy(), 'r--', label='Predicted', alpha=0.8)
        ...
        ax.set_ylabel('Count')
        ax.set_title(f'Error Distribution (μ={errors.mean():.3f}, σ={errors.std():.3f})')
        ax.legend()
...
def log_latent_reward_structure(
    ...
    with torch.no_grad():
        ...
        ax = axes[1]
        scatter = ax.scatter(z_2d[:, 0], z_2d[:, 1], c=rew_np, cmap='RdYlGn', 
                            s=10, alpha=0.5)
...
def log_action_conditioned_prediction(
    ...
    with torch.no_grad():
        ...
        # 5. Random actions
        action_sequences.append(torch.randn(1, horizon, act_dim, device=device).clamp(-1, 1))
        ...
        # Plot
        fig, axes = plt.subplots(n_actions + 1, horizon, figsize=(2*horizon, 2*(n_actions+1)))
        ...
        pred_stack = torch.stack([p.reshape(-1) for p in predictions])  # [n_actions, horizon*C*H*W]
...
def log_latent_dynamics(
    ...
    with torch.no_grad():
        ...
        for b in range(min(B, 10)):
            ax.plot(z_2d[b, :, 0], z_2d[b, :, 1], '-', color=colors[b], alpha=0.7, linewidth=1)
            ax.scatter(z_2d[b, 0, 0], z_2d[b, 0, 1], color=colors[b], s=50, marker='o', edgecolors='black')  # Start
            ax.scatter(z_2d[b, -1, 0], z_2d[b, -1, 1], color=colors[b], s=50, marker='x')  # End
        ax.set_xlabel('PCA 1')
        ax.set_ylabel('PCA 2')
        ...
        scatter = ax.scatter(z_2d.reshape(-1, 2)[:, 0], z_2d.reshape(-1, 2)[:, 1],
                            c=time_colors, cmap='viridis', s=10, alpha=0.5)
        plt.colorbar(scatter, ax=ax, label='Time step')
        ax.set_xlabel('PCA 1')
        ax.set_ylabel('PCA 2')
