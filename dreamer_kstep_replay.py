...
def sample_sequences_with_indices(
    ...
    batch = ReplayBuffer.Batch(replay.obs[seq_idx], replay.actions[seq_idx], replay.rews[seq_idx], replay.dones[seq_idx])
    return batch, seq_idx


class ReplayCellGraph:
    def __init__(self, geodesic):
        self.geodesic = geodesic
        self.n_cells = int(geodesic.n_free)
        self.adj = np.full((self.n_cells, self.n_cells), np.inf, dtype=np.float32)
        np.fill_diagonal(self.adj, 0.0)
        self.edge_count = 0
        self.dist = np.full((self.n_cells, self.n_cells), np.inf, dtype=np.float32)
        np.fill_diagonal(self.dist, 0.0)

    def add_transition(self, pos_a: np.ndarray, pos_b: np.ndarray):
        ca = int(_positions_to_cell_indices(self.geodesic, np.asarray(pos_a, dtype=np.float32).reshape(1, -1))[0])
        cb = int(_positions_to_cell_indices(self.geodesic, np.asarray(pos_b, dtype=np.float32).reshape(1, -1))[0])
        if ca == cb:
            return
        if not np.isfinite(self.adj[ca, cb]):
            self.edge_count += 1
        self.adj[ca, cb] = 1.0
        self.adj[cb, ca] = 1.0

    def rebuild_shortest_paths(self):
        mat = lil_matrix((self.n_cells, self.n_cells), dtype=np.float32)
        ii, jj = np.where(np.isfinite(self.adj) & (self.adj > 0))
        if len(ii) > 0:
            mat[ii, jj] = 1.0
        self.dist = shortest_path(mat.tocsr(), directed=False, unweighted=True).astype(np.float32)
        np.fill_diagonal(self.dist, 0.0)

    def sample_distance_pairs(self, pos_seq: np.ndarray, n_pairs: int = 256):
        flat_pos = pos_seq.reshape(-1, pos_seq.shape[-1])
        N = flat_pos.shape[0]
        if N < 4:
            return None
        cells = _positions_to_cell_indices(self.geodesic, flat_pos)
        ii = np.random.randint(0, N, size=n_pairs)
        jj = np.random.randint(0, N, size=n_pairs)
        keep = ii != jj
        ii, jj = ii[keep], jj[keep]
        d = self.dist[cells[ii], cells[jj]]
        keep = np.isfinite(d) & (d > 0)
        if int(np.sum(keep)) < 8:
            return None
        return ii[keep].astype(np.int64), jj[keep].astype(np.int64), d[keep].astype(np.float32)


def replay_graph_geo_loss(geo_head, h_seq, s_seq, pos_seq, replay_graph, n_pairs=256, stress_eps=0.1, uniformity_weight=0.05, uniformity_t=2.0):
    sample = replay_graph.sample_distance_pairs(pos_seq, n_pairs=n_pairs)
    if sample is None:
        zero = h_seq.new_zeros(())
        return zero, {"replay_geo/loss": 0.0, "replay_geo/stress": 0.0, "replay_geo/uniformity": 0.0, "replay_geo/n_pairs": 0.0, "replay_geo/scale": 0.0}
    ii, jj, d_np = sample
    g = geo_head(h_seq.detach().reshape(-1, h_seq.shape[-1]), s_seq.detach().reshape(-1, s_seq.shape[-1]))
    d_t = torch.tensor(d_np, device=h_seq.device, dtype=torch.float32)
    ii_t = torch.tensor(ii, device=h_seq.device, dtype=torch.long)
    jj_t = torch.tensor(jj, device=h_seq.device, dtype=torch.long)
    gi = g[ii_t]
    gj = g[jj_t]
    d_lat = torch.norm(gi - gj, dim=-1)
    weight = 1.0 / (d_t + stress_eps)
    scale = ((d_lat.detach() * d_t).sum() / (d_t.pow(2).sum() + 1e-6)).clamp(min=1e-4)
    stress = (weight * (d_lat - scale * d_t).pow(2)).mean()
    g_uni = torch.cat([gi, gj], dim=0)
    sq = torch.cdist(g_uni, g_uni).pow(2)
    mask = torch.eye(sq.size(0), device=sq.device, dtype=torch.bool)
    sq = sq.masked_fill(mask, 1e9)
    loss_uni = torch.logsumexp(-uniformity_t * sq, dim=1).mean()
    loss = stress + uniformity_weight * loss_uni
    return loss, {"replay_geo/loss": float(loss.item()), "replay_geo/stress": float(stress.item()), "replay_geo/uniformity": float(loss_uni.item()), "replay_geo/n_pairs": float(len(d_np)), "replay_geo/scale": float(scale.item())}


@torch.no_grad()
def compute_geo_head_correlation(encoder, rssm, geo_head, obs_buffer, pos_buffer, target_dist, geodesic, prefix, device, bit_depth, n_pairs=2000):
    encoder.eval(); rssm.eval(); geo_head.eval()
    N = len(pos_buffer)
    if N < 20:
        return {f"{prefix}/spearman": 0.0, f"{prefix}/n_pairs": 0}
    ...
    keep = ii != jj
    ii, jj = ii[keep], jj[keep]
    def encode_geo(indices):
        outs = []
        for s in range(0, len(indices), 256):
            batch_idx = indices[s:s+256]
            ...
            outs.append(geo_head(h_batch, s_batch).cpu().numpy())
        return np.concatenate(outs, axis=0)
    g_i = encode_geo(ii)
    g_j = encode_geo(jj)
    d_lat = np.linalg.norm(g_i - g_j, axis=-1)
    cell_i = _positions_to_cell_indices(geodesic, pos_buffer[ii])
    cell_j = _positions_to_cell_indices(geodesic, pos_buffer[jj])
    d_target = np.array([float(target_dist[int(ci), int(cj)]) for ci, cj in zip(cell_i, cell_j)], dtype=np.float32)
    finite = np.isfinite(d_lat) & np.isfinite(d_target) & (d_lat > 0) & (d_target > 0)
    if int(np.sum(finite)) < 10:
        return {f"{prefix}/spearman": 0.0, f"{prefix}/n_pairs": 0}
    rho = sp_stats.spearmanr(d_lat[finite], d_target[finite]).correlation
    if rho is None or not np.isfinite(rho):
        rho = 0.0
    return {f"{prefix}/spearman": float(rho), f"{prefix}/n_pairs": int(np.sum(finite)), f"{prefix}/mean_latent_dist": float(np.mean(d_lat[finite])), f"{prefix}/mean_target_dist": float(np.mean(d_target[finite]))}
...
def build_parser():
    ...
    p.add_argument("--geo_mode", type=str, default="baseline",
                   choices=["baseline", "vicreg", "straight", "georeg", "georeg_cov", "kstep", "kstep_replay"],
                   help="baseline | vicreg | straight | georeg (both) | georeg_cov (both + coverage) | kstep (InfoNCE reachability) | kstep_replay (kstep + separate replay-graph geo head)")
    ...
    p.add_argument("--replay_geo_weight", type=float, default=1.0,
                   help="Weight for separate replay-graph geo-head loss")
    p.add_argument("--replay_geo_pairs", type=int, default=256,
                   help="Number of replay-graph distance pairs per batch for geo head")
    p.add_argument("--replay_graph_refresh", type=int, default=10,
                   help="Rebuild replay-cell shortest paths every N episodes")
    p.add_argument("--geo_head_lr", type=float, default=3e-4,
                   help="Learning rate for separate replay-graph geo head")
    p.add_argument("--geo_eval_pairs", type=int, default=2000,
                   help="Pairs for periodic geo-head evaluation")
...
def main(args):
    ...
    use_kstep = args.geo_mode in ("kstep", "kstep_replay")
    use_replay_geo = args.geo_mode == "kstep_replay"
    ...
    print(f"  Replay-graph geo head: {use_replay_geo} (weight={args.replay_geo_weight})")
    if use_kstep:
    ...
    geo_head = GeoEncoder(args.deter_dim, args.stoch_dim, geo_dim=32, hidden_dim=256).to(device)
    ...
    replay_pos = np.zeros((args.replay_capacity, 2), dtype=np.float32)
    current_episode_id = 0
    replay_graph = ReplayCellGraph(geodesic)
    replay_graph.rebuild_shortest_paths()
    best_geo_score = -1e9
    ...
    for _ in range(args.seed_episodes):
        ...
        while not done:
            ...
            replay_pos[write_idx] = prev_pos
            replay_graph.add_transition(prev_pos, env.agent_pos.copy())
            obs = next_obs
            total_steps += 1
    ...
    for episode in range(args.max_episodes):
        ...
        while not done:
            ...
            if total_steps % args.collect_interval == 0 and replay.size > (args.seq_len + 2):
                ...
                sum_vicreg = sum_straight = sum_coverage = sum_kstep = sum_replay_geo = 0.0
                vicreg_info_accum: dict[str, float] = {}
                straight_info_accum: dict[str, float] = {}
                kstep_info_accum: dict[str, float] = {}
                replay_geo_info_accum: dict[str, float] = {}

                for _ in range(args.train_steps):
                    batch, seq_idx = sample_sequences_with_indices(replay, args.batch_size, args.seq_len + 1)
                    pos_seq_np = replay_pos[seq_idx]
                    ...
                    l_replay_geo = torch.tensor(0.0, device=device)
                    if use_replay_geo and total_steps >= args.kstep_min_steps:
                        l_replay_geo, replay_geo_info = replay_graph_geo_loss(
                            geo_head, h_seq, s_seq, pos_seq_np[:, 1:T + 1], replay_graph,
                            n_pairs=args.replay_geo_pairs,
                        )
                        geo_opt.zero_grad(set_to_none=True)
                        (args.replay_geo_weight * l_replay_geo).backward()
                        torch.nn.utils.clip_grad_norm_(geo_head.parameters(), args.grad_clip)
                        geo_opt.step()
                        for k, v in replay_geo_info.items():
                            replay_geo_info_accum[k] = replay_geo_info_accum.get(k, 0.0) + v
                    ...
                    sum_replay_geo += float(l_replay_geo.item())
                ...
                if use_kstep:
                    ...
                    for k, v in kstep_info_accum.items():
                        writer.add_scalar(f"loss/{k}", v / n_ts, total_steps)
                if use_replay_geo:
                    writer.add_scalar("loss/replay_geo_total", sum_replay_geo / n_ts, total_steps)
                    writer.add_scalar("loss/replay_geo_weighted", args.replay_geo_weight * sum_replay_geo / n_ts, total_steps)
                    for k, v in replay_geo_info_accum.items():
                        writer.add_scalar(f"loss/{k}", v / n_ts, total_steps)
                if use_coverage:
        ...
        if use_replay_geo and args.replay_graph_refresh > 0 and (episode + 1) % args.replay_graph_refresh == 0:
            replay_graph.rebuild_shortest_paths()
            writer.add_scalar("replay_graph/edge_count", float(replay_graph.edge_count), total_steps)
        ...
        if args.geo_corr_interval > 0 and (episode + 1) % args.geo_corr_interval == 0:
            if len(obs_geo_buffer) >= 100:
                ...
                if use_replay_geo and replay_graph.edge_count > 0:
                    geo_head_replay = compute_geo_head_correlation(
                        encoder, rssm, geo_head, obs_arr, pos_arr, replay_graph.dist, geodesic,
                        prefix="geo_head_replay", device=device, bit_depth=args.bit_depth, n_pairs=args.geo_eval_pairs)
                    geo_head_oracle = compute_geo_head_correlation(
                        encoder, rssm, geo_head, obs_arr, pos_arr, geodesic.dist_matrix, geodesic,
                        prefix="geo_head_oracle", device=device, bit_depth=args.bit_depth, n_pairs=args.geo_eval_pairs)
                    merged = dict(geo_head_replay)
                    merged.update(geo_head_oracle)
                    for k, v in merged.items():
                        writer.add_scalar(k, v, total_steps)
                    score = float(geo_head_oracle.get("geo_head_oracle/spearman", -1e9))
                    if score > best_geo_score:
                        best_geo_score = score
                        torch.save({
                            "encoder": encoder.state_dict(),
                            "rssm": rssm.state_dict(),
                            "geo_head": geo_head.state_dict(),
                            "episode": episode + 1,
                            "total_steps": total_steps,
                            "best_geo_score": best_geo_score,
                        }, os.path.join(out_dir, "best_geo_bundle.pt"))
                        writer.add_scalar("geo_head/best_oracle_spearman", best_geo_score, total_steps)
                    print(f"    [geo head replay] ρ={geo_head_replay.get('geo_head_replay/spearman', 0.0):.4f}  [geo head oracle] ρ={geo_head_oracle.get('geo_head_oracle/spearman', 0.0):.4f}")
